...
export const OrderProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const addReview = async (orderId: string, rating: number, comment: string) => {
    ...
    const orderRef = doc(db, 'restaurants', restaurantId, 'orders', orderId); const orderSnap = await getDoc(orderRef); if (!orderSnap.exists() || (orderSnap.data() as Order).customerId !== currentUser.uid) throw new Error('Order ownership verification failed');
    await addDoc(collection(db, 'restaurants', restaurantId, 'reviews'), { restaurantId, orderId, rating, comment: comment.slice(0, 1000), createdAt: serverTimestamp() });
  };
