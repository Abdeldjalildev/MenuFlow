...
import { normalizeOrderItem, normalizeOrderStatus, type CanonicalOrderStatus, type CanonicalOrderItem } from '../services/orderDomain';
...
export interface PlaceOrderResult { orderId: string; orderNumber: number; orderNumberDate?: string; subtotal?: number; discountAmount?: number; totalAmount?: number; orderSource?: string }
...
interface OrderContextType { orders: Order[]; placeOrder: (items: OrderItem[], tableNumber: string, deliveryData?: DeliveryData | null, totalAmount?: number, extraOptions?: PlaceOrderExtraOptions) => Promise<PlaceOrderResult>; appendToOrder: (orderId: string, newItems: OrderItem[]) => Promise<void>; updateOrderStatus: (orderId: string, newStatus: OrderStatus) => Promise<void>; addReview: (orderId: string, rating: number, comment: string) => Promise<void>; claimOrderForDriver: (orderId: string, driverId: string, driverName: string) => Promise<{ success: boolean; message?: string; error?: unknown }> }
const defaultContext: OrderContextType = { orders: [], placeOrder: async () => { throw new Error('OrderContext not initialized'); }, appendToOrder: async () => {}, updateOrderStatus: async () => {}, addReview: async () => {}, claimOrderForDriver: async () => ({ success: false, message: 'OrderContext not initialized' }) };
...
export const OrderProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const placeOrder = async (items: OrderItem[], tableNumber: string, deliveryData?: DeliveryData | null, _totalAmount?: number, extraOptions?: PlaceOrderExtraOptions): Promise<PlaceOrderResult> => {
    ...
    const createOrder = httpsCallable<Record<string, unknown>, PlaceOrderResult>(getFunctions(), 'createOrder');
    const response = await createOrder({ restaurantId: targetRestaurant, items: canonicalItems, tableNumber: tableNumber || '0', deliveryData: deliveryData || null, orderSource: 'customer', mutationId: newMutationId() });
  ...
  const claimOrderForDriver = async (orderId: string, _driverId: string, _driverName: string) => {
    try { const mutateOrder = httpsCallable(getFunctions(), 'mutateOrder'); await mutateOrder({ operation: 'driver_claim', restaurantId, orderId }); return { success: true }; } catch (error) { return { success: false, error }; }
  };
