...
import { Check, LogOut, ShoppingCart, X } from 'lucide-react';
...
const text: Record<Lang, Record<string, string>> = {
  ar: { title: 'واجهة النادل', subtitle: 'إنشاء طلب للطاولة', table: 'رقم الطاولة', placeholder: 'مثال: 12', cart: 'السلة', empty: 'السلة فارغة', submit: 'إرسال الطلب للمطبخ', submitting: 'جاري الإرسال...', success: 'تم إنشاء الطلب بنجاح', order: 'رقم الطلب', error: 'تعذر تحميل البيانات', auth: 'جلسة النادل غير صالحة', noMenu: 'لا توجد عناصر متاحة في القائمة', logout: 'تسجيل الخروج', addToCart: 'إضافة', specialNote: 'ملاحظة خاصة', currency: 'دج', confirmTable: 'تأكيد الطاولة', changeTable: 'تغيير الطاولة', confirmPrompt: 'هل تريد إرسال الطلب إلى هذه الطاولة؟', selected: 'الطاولة المؤكدة', invalidTable: 'أدخل رقم طاولة صالحاً.', confirmBeforeSubmit: 'أكد رقم الطاولة قبل الإرسال.' },
  en: { title: 'Waiter', subtitle: 'Create an order for a table', table: 'Table number', placeholder: 'e.g. 12', cart: 'Cart', empty: 'Cart is empty', submit: 'Send order to kitchen', submitting: 'Submitting...', success: 'Order created successfully', order: 'Order number', error: 'Unable to load data', auth: 'Invalid waiter session', noMenu: 'No available menu items', logout: 'Sign out', addToCart: 'Add', specialNote: 'Special note', currency: 'DZD', confirmTable: 'Confirm table', changeTable: 'Change table', confirmPrompt: 'Send this order to the selected table?', selected: 'Confirmed table', invalidTable: 'Enter a valid table number.', confirmBeforeSubmit: 'Confirm the table number before submitting.' },
  fr: { title: 'Serveur', subtitle: 'Créer une commande pour une table', table: 'Numéro de table', placeholder: 'ex. 12', cart: 'Panier', empty: 'Panier vide', submit: 'Envoyer en cuisine', submitting: 'Envoi...', success: 'Commande créée avec succès', order: 'Numéro de commande', error: 'Impossible de charger les données', auth: 'Session serveur invalide', noMenu: 'Aucun article disponible', logout: 'Se déconnecter', addToCart: 'Ajouter', specialNote: 'Note spéciale', currency: 'DZD', confirmTable: 'Confirmer la table', changeTable: 'Changer de table', confirmPrompt: 'Envoyer cette commande à la table sélectionnée ?', selected: 'Table confirmée', invalidTable: 'Saisissez un numéro de table valide.', confirmBeforeSubmit: 'Confirmez le numéro de table avant l’envoi.' },
};

const makeMutationId = () => typeof crypto !== 'undefined' && typeof crypto.randomUUID === 'function' ? crypto.randomUUID() : `${Date.now()}_${Math.random().toString(36).slice(2)}`;
...
export const WaiterExperience: React.FC = () => {
  ...
  const submit = async () => {
    if (!restaurantId || !confirmedTable || count === 0 || submitting) return;
    setSubmitting(true); setError(''); setSuccess(null);
    const mutationId = pendingMutationId || makeMutationId();
    if (!pendingMutationId) setPendingMutationId(mutationId);
    try {
      const items = Object.entries(cart).map(([menuItemId, quantity]) => ({ menuItemId, quantity, note: notes[menuItemId] || '' }));
      const createOrder = httpsCallable(functions, 'createOrder');
      const result = await createOrder({ restaurantId, tableNumber: confirmedTable, orderSource: 'waiter', mutationId, items });
  ...
  return <div className="min-h-screen bg-slate-50 text-slate-900" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
    ...
    <main className="max-w-3xl mx-auto px-4 py-5 space-y-5">
      <section className="bg-white rounded-2xl border p-4"><label htmlFor="waiter-table" className="block text-sm font-bold mb-2">{t.table}</label><div className="flex gap-2"><input id="waiter-table" value={tableNumber} onChange={e => setTable(e.target.value)} placeholder={t.placeholder} className="flex-1 border rounded-xl px-3 py-2.5" dir="ltr" aria-invalid={tableNumber.length > 0 && !validTable} />{confirmedTable ? <button type="button" onClick={() => setConfirmedTable(null)} className="px-3 py-2 rounded-xl border font-bold text-sm flex items-center gap-1"><X size={15} /> {t.changeTable}</button> : <button type="button" onClick={confirmCurrentTable} disabled={!validTable} className="px-3 py-2 rounded-xl bg-slate-900 text-white font-bold text-sm disabled:opacity-40 flex items-center gap-1"><Check size={15} /> {t.confirmTable}</button>}</div>{confirmedTable && <p className="text-xs text-emerald-700 mt-2 font-semibold">{t.selected}: {confirmedTable}</p>}{tableNumber && !validTable && <p className="text-xs text-red-600 mt-2">{t.invalidTable}</p>}</section>
      ...
      <section className="sticky bottom-4 bg-white rounded-2xl border shadow-lg p-4"><div className="flex items-center justify-between gap-4"><div className="flex items-center gap-2 font-bold"><ShoppingCart size={18} /> {t.cart}: {count}</div><button disabled={!confirmedTable || count === 0 || submitting} onClick={() => void submit()} className="px-4 py-2.5 rounded-xl bg-slate-900 text-white font-bold text-sm disabled:opacity-40">{submitting ? t.submitting : t.submit}</button></div>{count === 0 && <p className="text-xs text-slate-500 mt-2">{t.empty}</p>}{count > 0 && !confirmedTable && <p className="text-xs text-amber-700 mt-2">{t.confirmBeforeSubmit}</p>}</section>
    </main>
  </div>;
};
