...
interface AnalyticsDashboardProps { lang?: Language; }
interface ChartItem { name: string; sales: number; }
  ...
  const chartData = orders
    ...
    .reduce<ChartItem[]>((acc, order) => {
      ...
      order.items.forEach((item: OrderItem) => {
        ...
        if (existing) existing.sales += quantity;
        else acc.push({ name, sales: quantity });
      });
      return acc;
    }, []);
      ...
      <div className="h-64">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={chartData}>
            ...
            <Tooltip formatter={(value) => [value, t.salesCount]} />
