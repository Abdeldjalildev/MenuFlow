...
export const Reports: React.FC<ReportsProps> = (props) => {
  ...
  const hasRestaurantId = Boolean(localStorage.getItem('restaurantId'));

  const [totalExpenses, setTotalExpenses] = useState<number | null>(hasRestaurantId ? null : 0);
  const [totalWaste, setTotalWaste] = useState<number | null>(hasRestaurantId ? null : 0);
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(qReviews, (snapshot) => {
      ...
      const positive = docs.filter(d => Number(d.rating) >= 4).length;
  ...
  const totalRevenue = orders
    ...
    .reduce((sum, order) => {
      ...
      const orderTotal = order.items.reduce((orderSum, item) => {
        const itemWithMenu = item as typeof item & { menuItem?: { price?: number } };
        const price = Number(itemWithMenu.menuItem?.price ?? item.price ?? 0);
