...
export const DigitalReceipt: React.FC<DigitalReceiptProps> = ({ 
  ...
  // دالة الأمان لجلب النصوص المترجمة
  ...
  // 🎯 استخراج تاريخ الفاتورة الحقيقي المكتوب في Firestore
  const getFormattedDate = () => {
    try {
      let rawDate = order.createdAt;
  ...
  // 🎯 جلب مصفوفة العناصر بأمان تام (سواء كانت مسجلة كـ items أو cart)
  const itemsList = Array.isArray(order.items) && order.items.length > 0 
    ? order.items 
    : (Array.isArray(order.cart) ? order.cart : []);

  // 🎯 حساب المجموع تلقائياً في حال كانت القيمة المسجلة 0 أو غير موجودة
  ...
  const finalTotal = (order.totalAmount || order.totalPrice) ? (order.totalAmount || order.totalPrice) : calculatedTotal;

  // 🎯 استخراج رقم الطاولة بشكل صحيح (مع دعم tableNumber)
    ...
    >
      ...
      <div className="pt-4 flex justify-between items-center">
        ...
        <span className="text-2xl font-black text-emerald-400">
         {(finalTotal || 0).toLocaleString()} <span className="text-sm font-bold">{safeText(t?.currency, 'د.ج')}</span>
        </span>
      </div>
