...
export const CustomerMenu: React.FC = () => {
  ...
  const prepareOrderPayloadData = () => {
    ...
    const formattedItems = Object.keys(cart).map(id => {
      ...
      const quantity = cart[id]; const originalPrice = Number(item?.price || 0); const price = activeDiscount > 0 ? originalPrice * (1 - activeDiscount / 100) : originalPrice; totalAmount += price * quantity;
      const nameAr = item?.name?.ar || '';
      return { id, menuItemId: id, recipeId: item?.recipeId || null, name: nameAr || 'وجبة', nameAr, price, originalPrice, quantity, note: notes[id] || '', image: item?.image || '' };
    });
  ...
  const handleCheckout = async () => {
    ...
    if (currentOrder) { await appendToOrder(currentOrder.id, formattedItems); setIsOrderPlaced(true); clearCart(); return; }
    if (currentTable && currentTable !== '0') { await placeOrder(formattedItems, currentTable, null, totalAmount, { appliedDiscountPercent: activeDiscount, restaurantId }); setIsOrderPlaced(true); clearCart(); } else setShowDeliveryForm(true);
  };
  const handleFinalSubmit = async (deliveryInfo: { name: string; address: string; phone: string }) => {
    const { formattedItems, totalAmount } = prepareOrderPayloadData();
    let driverName = 'غير محدد'; let driverPhone = '';
    try { const snap = await getDocs(query(collection(db, 'staff'), where('restaurantId', '==', restaurantId), where('role', '==', 'delivery'), where('status', '==', 'active'))); if (!snap.empty) { const d = snap.docs[0].data(); driverName = d.name || driverName; driverPhone = d.phone || ''; } } catch (error) { console.warn('Could not resolve delivery staff:', error); }
    await placeOrder(formattedItems, '0', deliveryInfo, totalAmount, { appliedDiscountPercent: activeDiscount, restaurantId, driverName, driverPhone });
    setIsOrderPlaced(true); setShowDeliveryForm(false); clearCart();
  };
  ...
  return <div className="min-h-screen pb-24 bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 relative" dir={lang === 'ar' ? 'rtl' : 'ltr'} style={{ backgroundColor: theme.menuBgColor || '#0f172a', backgroundImage: theme.menuBgType === 'image' && theme.menuBgImage ? `url('${theme.menuBgImage}')` : 'none', backgroundSize: 'cover', backgroundPosition: 'center', backgroundRepeat: 'no-repeat', backgroundAttachment: 'fixed' }}>
    <div className="relative z-10">
      <MenuHeader currentTable={currentTable} lang={lang} themeColor={effectivePrimaryColor} logoUrl={theme.logoUrl} bannerUrl={theme.menuBannerUrl} t={t} onOpenReceipts={() => setShowReceipts(true)} />
      {showReceipts ? <MyReceipts lang={lang} activeOrderId={currentOrder?.id} onBack={() => setShowReceipts(false)} /> : <main className="max-w-md mx-auto px-4 mt-4">
        {activeDiscount > 0 && !currentOrder && <div className="mb-4 bg-emerald-500 text-white p-3.5 rounded-2xl shadow-lg flex items-center justify-between"><div className="flex items-center gap-2.5"><Tag className="w-5 h-5" /><div><h4 className="font-bold text-sm">تهانينا! لديك خصم مفعّل 🎉</h4><p className="text-xs text-emerald-100">تم تطبيق تخفيض {activeDiscount}% على جميع الوجبات!</p></div></div><span className="bg-white text-emerald-600 font-extrabold text-xs px-2.5 py-1 rounded-xl">-{activeDiscount}%</span></div>}
        <div className="mb-6 text-center"><h2 className="text-xl font-bold">{t('appName')}</h2><p className="text-sm text-slate-600 dark:text-slate-400">{t('welcomeMessage')}</p></div>
        <div className="mb-6"><AIChat lang={lang} menuItems={menuItems} t={t} themeColor={effectivePrimaryColor} /></div>
        <MenuOrOrderManager showDeliveryForm={showDeliveryForm} isOrderPlaced={isOrderPlaced} currentOrder={currentOrder ?? null} onDeliveryConfirm={(name, address, phone) => handleFinalSubmit({ name, address, phone })} t={t} themeColor={effectivePrimaryColor} />
        <CategoryTabs items={menuItems} activeCategory={activeCategory} setActiveCategory={setActiveCategory} themeColor={effectivePrimaryColor} t={t} />
        <MenuGrid items={filteredItems} cart={cart} notes={notes} onAddToCart={handleAddToCart} onNoteChange={updateNote} themeColor={effectivePrimaryColor} lang={lang} t={t} activeDiscount={activeDiscount} />
        <CheckoutBar cartCount={cartCount} handleCheckout={handleCheckout} themeColor={effectivePrimaryColor} hasActiveOrder={!!currentOrder} t={t} activeDiscount={activeDiscount} />
      </main>}
    </div>
  </div>;
};
