...
export const OrderTracking: React.FC<{ t: (key: string) => string }> = ({ t }) => {
  ...
  const currentOrder = orders
    .filter(order => order.tableNumber === currentTable && order.status !== 'completed' && !!uid && order.customerId === uid)
    .sort((a, b) => {
  ...
  const finish = async () => { try { await updateOrderStatus(currentOrder.id, 'completed'); } catch (error) { console.error('Error finishing order:', error); } };
