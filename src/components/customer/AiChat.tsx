...
export const AIChat: React.FC<Props> = ({ menuItems, lang, t, themeColor }) => {
  ...
  const handleAskAI = async () => {
    ...
    try {
      ...
      // 🧠 تلخيص وتحويل كل تفاصيل المنيو إلى نص دقيق وشامل يفهمه الـ AI
      const menuDetailsSummary = menuItems && menuItems.length > 0 
        ? menuItems.map((item: any, index) => {
            const name = typeof item.name === 'object' ? (item.name[lang] || item.name.ar || item.name.en || '') : (item.nameAr || item.name || '');
            const price = item.price || 0;
            const category = item.category || 'عام';
            const description = typeof item.description === 'object' ? (item.description[lang] || item.description.ar || '') : (item.description || '');
      ...
      // 🎯 برومبت قوي وشامل يعرفه بكل تفاصيل المنيو والمطعم
    ...
    <div className="bg-white dark:bg-slate-900 p-5 rounded-2xl shadow-sm border border-indigo-100 dark:border-slate-800 mb-6" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
      ...
      <div className="flex flex-col gap-2">
        ...
        <button 
          ...
          className={`px-4 py-2 rounded-xl text-sm font-medium transition text-white self-end w-full sm:w-auto shadow-sm active:scale-95 cursor-pointer ${
            !themeColor ? 'bg-indigo-600 hover:bg-indigo-700' : 'hover:opacity-90'
          }`}
        >
          {loading ? '...' : t('send')}
        </button>
