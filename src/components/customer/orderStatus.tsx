...
export const OrderStatus: React.FC<OrderStatusProps> = ({ status, t, isRated }) => {
  ...
  // التعديل الدقيق: الاكتفاء بحالة pending الحقيقية فقط وعدم لمس حجز السائق لتجنب أي تراجع في المراحل
  if (status === 'pending') {
    visualStatus = 'pending';
  } else if (status === 'preparing') {
    visualStatus = 'preparing';
  } else if (status === 'ready' || status === 'ready_for_payment' || status === 'ready_for_delivery') {
    visualStatus = 'ready';
  } else if (status === 'on_the_way') {
    visualStatus = 'on_the_way';
  } else if (status === 'delivered_unpaid' || status === 'paid' || status === 'completed') {
    visualStatus = status === 'TrackDone' ? 'review' : 'paid';
  ...
  return (
    <div className="text-center p-6 bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm">
      <motion.div 
        ...
        className="w-16 h-16 bg-indigo-50 dark:bg-indigo-950 text-indigo-600 rounded-full flex items-center justify-center mx-auto mb-4"
      >
        ...
        {stages.map((stage, i) => (
          <div key={stage.id} className="relative z-10 flex flex-col items-center">
            <motion.div 
              initial={false}
              animate={{ 
                backgroundColor: (i <= currentStageIndex || isRated) ? '#4f46e5' : '#e2e8f0',
                scale: i === currentStageIndex ? 1.1 : 1
              }}
              className="w-8 h-8 rounded-full flex items-center justify-center border-2 border-white dark:border-slate-900 shadow-sm text-white text-xs font-bold"
            >
              {i + 1}
            </motion.div>
