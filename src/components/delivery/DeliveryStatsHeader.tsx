...
export const DeliveryStatsHeader: React.FC<Props> = ({ orders, lang = 'ar' }) => {
  ...
  const texts = {
    ar: { title: 'إجمالي النقدية المعلقة بحوزتك', subtext: 'المبالغ المجمعة من الطلبات الموصلة وبانتظار التسليم للكاشير', ordersCount: 'طلبات موصلة معلقة', currency: 'د.ج' },
    fr: { title: 'Cash En Attente De Remise', subtext: 'Montants encaisés des commandes livrées à remettre à la caisse', ordersCount: 'Commandes livrées non réglées', currency: 'DZD' },
    en: { title: 'Pending Cash In Hand', subtext: 'Collected cash from delivered orders pending cashier settlement', ordersCount: 'Delivered Pending Orders', currency: 'DZD' },
  };
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="text-3xl font-black tracking-tight mt-0.5">{totalPendingCash.toLocaleString()} <span className="text-lg font-bold text-emerald-200">{t.currency}</span></div>
