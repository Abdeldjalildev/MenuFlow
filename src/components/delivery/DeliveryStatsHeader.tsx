...
export const DeliveryStatsHeader: React.FC<Props> = ({ orders, lang = 'ar' }) => {
  ...
  // دالة حساب المبلغ الدقيق لترتيب الأولوية تماماً مثل DeliveryDashboard
  const getOrderFinalPrice = (order: Order): number => {
    // 1. حساب المجموع من الأصناف يدويًا للضمان
    ...
    // 2. مطابقة نفس متوالية التقييم المستخدمة في الكروت
    const price = order.totalPrice ?? order.totalAmount ?? order.total ?? calculatedTotalFromItems;
    return Number(price || 0);
  };

  // ✅ الفلترة الدقيقة: فقط الطلبات التي سلمها السائق بالفعل ولم يدفها للكاشير بعد
  const deliveredUnpaidOrders = orders.filter(
    (order) => order.status === 'delivered_unpaid'
  );

  // حساب النقدية الفعلية المجمعة في جيب السائق حالياً
  const totalPendingCash = deliveredUnpaidOrders.reduce((sum, order) => {
    return sum + getOrderFinalPrice(order);
  }, 0);
  ...
  return (
    <div className="w-full bg-linear-to-r from-emerald-600 to-teal-700 text-white rounded-3xl p-5 mb-6 shadow-lg border border-emerald-500/30 transition-all">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        {/* التفاصيل الرئيسية للمبلغ */}
        <div className="flex items-center gap-4">
          <div className="p-3.5 bg-white/10 backdrop-blur-md rounded-2xl border border-white/20 text-2xl">
            💵
          </div>
          <div>
        ...
        <div className="bg-white/10 backdrop-blur-md px-4 py-2.5 rounded-2xl border border-white/15 flex items-center gap-3 self-stretch sm:self-auto justify-between">
          ...
          <span className="text-lg font-black bg-white text-emerald-800 px-3 py-0.5 rounded-xl shadow-sm">
