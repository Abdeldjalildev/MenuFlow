...
const TRANSITIONS = {
  pending: new Set(['preparing']), preparing: new Set(['driver_claimed', 'ready_for_payment', 'ready']),
  driver_claimed: new Set(['ready_for_delivery']), ready: new Set(['ready_for_payment']),
  ready_for_payment: new Set(['ready_for_delivery', 'paid']), ready_for_delivery: new Set(['on_the_way']),
  on_the_way: new Set(['delivered_unpaid']), delivered_unpaid: new Set(['paid']), paid: new Set(['completed']), completed: new Set(),
};
...
function normalizeIngredient(recipeIngredient) {
  const inventoryItemId = recipeIngredient?.inventoryItemId, quantity = Number(recipeIngredient?.quantity);
...
function normalizeOrderItems(items) {
  ...
  return items.map(item => {
    if (!item || typeof item !== 'object') throw new HttpsError('invalid-argument', 'Invalid order item.');
    const quantity = Number(item.quantity ?? item.qty ?? 1), price = Number(item.price ?? item.unitPrice ?? 0);
    if (!Number.isFinite(quantity) || quantity <= 0 || !Number.isFinite(price) || price < 0) throw new HttpsError('invalid-argument', 'Order item quantity and price must be valid numbers.');
...
exports.createOrder = onCall(async (request) => {
  ...
  if (!auth || auth.token?.firebase?.sign_in_provider !== 'anonymous') throw new HttpsError('unauthenticated', 'Anonymous customer authentication is required.');
  const restaurantId = request.data?.restaurantId, tableNumber = request.data?.tableNumber;
  const items = normalizeOrderItems(request.data?.items), deliveryData = normalizeDeliveryData(request.data?.deliveryData);
  const suppliedTotal = request.data?.totalAmount;
...
exports.transitionOrder = onCall(async (request) => {
  ...
  const role = auth.token?.role, restaurantId = auth.token?.restaurantId;
  ...
  const orderId = request.data?.orderId, newStatus = request.data?.newStatus;
  ...
  await db.runTransaction(async tx => {
    ...
    if (newStatus === 'preparing' && !order.inventoryDeducted) {
      ...
      inventorySnaps.forEach((snap, index) => {
        ...
        const data = snap.data(), current = Number(data.currentQuantity ?? data.quantity ?? 0), required = deductions.get(inventoryRefs[index].id) || 0;
      ...
      inventorySnaps.forEach((snap, index) => {
        const ref = inventoryRefs[index], current = Number(snap.data().currentQuantity ?? snap.data().quantity ?? 0);
