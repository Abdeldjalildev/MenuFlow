...
const orderRef = (db, restaurantId = RESTAURANT_A, id = 'order-a') =>
...
const validOrder = (status = 'pending') => ({
  ...
  status,
  totalAmount: 1200,
  createdAt: Timestamp.fromDate(new Date('2026-08-25T10:00:00Z')),
  driverId: null,
  driverName: null,
  isClaimed: false,
  isPaid: false,
...
async function seedOrder(status = 'pending', extra = {}) {
  await testEnv.withSecurityRulesDisabled(async context => {
    await setDoc(orderRef(context.firestore()), { ...validOrder(status), ...extra });
  });
}
...
test('Gate 5: every supported lifecycle transition is explicitly allowed to its responsible role', async () => {
  const cases = [
    ['Kitchen', 'kitchen-a', 'pending', 'preparing'],
    ['Kitchen', 'kitchen-a', 'preparing', 'ready'],
    ['Kitchen', 'kitchen-a', 'preparing', 'ready_for_payment'],
    ['Delivery', 'delivery-a', 'preparing', 'driver_claimed'],
    ['Delivery', 'delivery-a', 'driver_claimed', 'ready_for_delivery'],
    ['Delivery', 'delivery-a', 'ready_for_delivery', 'on_the_way'],
    ['Delivery', 'delivery-a', 'on_the_way', 'delivered_unpaid'],
    ['Cashier', 'cashier-a', 'ready_for_payment', 'paid'],
    ['Cashier', 'cashier-a', 'delivered_unpaid', 'paid'],
    ['Cashier', 'cashier-a', 'paid', 'completed'],
  ];

  for (const [role, uid, from, to] of cases) {
    await testEnv.clearFirestore();
    await seedOrder(from);
    const db = tenantUser(uid, RESTAURANT_A, role);
    const updates = { status: to, updatedAt: Timestamp.now() };
    if (role === 'Delivery' && from === 'preparing') {
      updates.driverId = uid;
      updates.driverName = 'Delivery A';
...
test('Gate 5: illegal lifecycle transitions are rejected for every operational role', async () => {
  const roles = [
    ['Kitchen', 'kitchen-a'], ['Cashier', 'cashier-a'], ['Delivery', 'delivery-a'], ['Admin', 'admin-a'],
  ];
  for (const [role, uid] of roles) {
    await testEnv.clearFirestore();
    await seedOrder('pending');
    const db = tenantUser(uid, RESTAURANT_A, role);
    await assertFails(updateDoc(orderRef(db), { status: 'paid', updatedAt: Timestamp.now() }));
  }
});

test('Gate 5: role boundaries reject otherwise-valid transitions owned by another role', async () => {
  const cases = [
    ['Kitchen', 'kitchen-a', 'ready_for_payment', 'paid'],
    ['Cashier', 'cashier-a', 'preparing', 'ready'],
    ['Delivery', 'delivery-a', 'ready_for_payment', 'paid'],
  ];
  for (const [role, uid, from, to] of cases) {
    await testEnv.clearFirestore();
    await seedOrder(from);
...
test('Gate 5: cashier can only mutate payment state while completing a legal payment transition', async () => {
  await seedOrder('ready_for_payment');
...
test('Gate 5: tenant isolation remains enforced for order reads and writes', async () => {
  await testEnv.withSecurityRulesDisabled(async context => {
    await setDoc(orderRef(context.firestore(), RESTAURANT_B, 'order-b'), { ...validOrder('pending'), restaurantId: RESTAURANT_B, customerId: 'customer-b' });
  });
  const db = tenantUser('admin-a', RESTAURANT_A, 'Admin');
...
test('Gate 5: inventory deduction is fail-closed, transactional, and idempotent by source contract', async () => {
  ...
  assert.match(source, /await db\.runTransaction\(async tx/);
  assert.match(source, /updates\.inventoryDeducted = true/);
  assert.match(source, /updates\.inventoryDeductedAt = new Date\(\)/);
  assert.match(source, /tx\.update\(orderRef, updates\)/);
});
...
test('Gate 5: concurrent order numbering remains transaction-backed and isolated by restaurant/day', async () => {
  ...
  assert.match(source, /orderNumberCounters\/\$\{orderNumberDate\}/);
  assert.match(source, /const counterSnap = await tx\.get\(counterRef\)/);
  assert.match(source, /tx\.set\(counterRef, \{ nextNumber: orderNumber \+ 1/);
  assert.match(source, /tx\.create\(orderRef, \{/);
  assert.match(helper, /timeZone: ORDER_NUMBER_TIME_ZONE/);
  assert.match(helper, /return counterData == null \? 1/);
});
